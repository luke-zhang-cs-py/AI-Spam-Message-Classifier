...
this family has shipped a figure that had quietly stopped being true. So the
figures are measured again here and compared.
...
What is checked statically, so it works in CI with no coverage run:
  ...
  * its statement count is what `coverage` counts, using this repo's own
    `.coveragerc` — which is what keeps the test files themselves out of
    the total, where they once padded it from 68% to 79%;
